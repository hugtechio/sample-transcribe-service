...
```bash
stripe-event-workflow$ sam build
```
...
```bash
stripe-event-workflow$ sam logs -n StockCheckerFunction --stack-name stripe-event-workflow --tail
```
...
```bash
stripe-event-workflow$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name stripe-event-workflow
```
